...
import com.dsim.http.ServerBootstrap.GuardianFailure
    ...
    Behaviors
      .setup[SelfUp] { ctx =>
        ...
        Behaviors.receive[SelfUp] { case (ctx, _ @SelfUp(_ /*state*/ )) =>
          ...
          val master = ClusterSingleton(ctx.system).init(
            SingletonActor(rdelivery.Master(cluster.selfMember.uniqueAddress.address), "master")
          )

          //Default {"flow-control-window":50,"only-flow-control":false,"resend-interval-max":"30s","resend-interval-min":"2s"}
          val worker = ctx.spawn(rdelivery.Worker(workerAddr), "worker")
          ...
          Behaviors.receiveSignal[SelfUp] {
            case (_, Terminated(`worker`)) =>
              ...
              CoordinatedShutdown(sys).run(GuardianFailure)
